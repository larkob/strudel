...
This guide is for people who are used to making music without a computer, who may have had some fun playing around 
with fancy hardware gadgets and are ready to move on to the next level: programming music!

If you're familiar with groove boxes like the Novation Circuit, then Strudel can be regarded as something 
akin to a programmable groove box with
...
want to suggest that this is how Strudel is meant to be used. I simply find it easier to learn something new by starting 
...
This is actually quite important in order to understand how Strudel works. Let's start with the most basic pattern 
...
Now Strudel plays four notes in the pattern, each at a quarter duration because multiple items in a pattern 
divide the cycle into multiple parts of equal duration. A pattern cannot exceed the cycle duration, so the more items 
you put in a pattern, the more they get squeezed together.

You can put "rests" in a pattern using the `~` symbol like this:
...
In summary: Patterns are an expression of events or "haps" (with a begin and end in the cycle) with arbitrary values that 
can be interpreted as sound controls. At the most basic level, this is simply what note is to be played, but you'll 
add further content to the event 
...
<MiniRepl tune={
`sound("hh") // same as s("hh").out()`
} />

The `sound` function specifies the sample sound you want to play. The dot chains functions together.
...
<MiniRepl tune={`
s("piano") .note("a2 b2 c3 d3 e3 f3 g3@2") .struct("x x x x x x x@2") 
.gain(0.5) .out()`
} />
...
Let's apply the same idea to the master track: I want a master variable which controls the overall `gain` and tempo. 
As such, it has to be a function that can be applied to the entire `stack` of patterns at the end.

<MiniRepl tune={
`await gist('larkob/4c933946b3e2f870b48d149ba11e8be7/raw/a1b52b8b546245618016057e5427d902bfe1efd2/strudel_helpers.js')
global({ scale: 'C4 major'})

const master = $ => $ .gain(0.5) .cpm(60)
...
You may notice that I'm using `cpm` instead of `bpm` because I haven't written the notes in a 4/4 time signature.
