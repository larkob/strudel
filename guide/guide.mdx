...
divide the cycle into multiple parts of equal duration. The total duration of items in a pattern cannot exceed 
the cycle duration of 1 second, so the more events you put in a pattern, the more they get squeezed together. 
...
A pattern can span multiple cycles, each of which lasts the cycle length of 1 second. This is typically done by 
con*cat*enating patterns, but there are other functions as well that can create multi-cycle patterns.

```
cat("c2 d2 ~ ~","c3 d3 ~ ~") // ['0/1 -> 1/4: c2', '1/4 -> 1/2: d2'], ['0/1 -> 1/4: c3', '1/4 -> 1/2: d3']
```

Last but not least, you can `stack` patterns together, so they appear as one.

```
stack("c2 d2 ~ ~","c3 d3 ~ ~") // ['0/1 -> 1/4: c2', '0/1 -> 1/4: c3', '1/4 -> 1/2: d2', '1/4 -> 1/2: d3']
```
...
I'm using the `sound` helper function as a replacement for `s("__").out()`, so the code becomes easier to read 
(and I have less to type).
...
stack(
  sound("sd") .struct("~ x ~ x"),
  sound("bd") .struct(bassdrum.pt03), // change this
)
.bpm(100, '4/4')`
} />
...
I've used `cat` for the first pattern even though it isn't needed, but oftentimes it's better to be consistent, 
plus it makes for better readability when the rhythmical structures are aligned. 
...
stack(
  sound("sd") .struct("~ x ~ x"),
  sound("bd") .struct(bassdrum.pt04), // change this
)
.bpm(100, '4/4')`
} />
...
//
stack(
  sound("oh") .struct(open_hihat.pt14), // change this
  sound("sd") .struct("~ x ~ x"),
  sound("bd") .struct("x ~ x ~"),
)
.bpm(100, '4/4')`
...
//
stack(
  sound("hh") .n(1) .struct(pattern.pt17.half),
  sound("oh") .struct(pattern.pt17.open),
  sound("sd") .struct("~ x ~ x"),
  sound("bd") .struct("x ~ x ~", "x ~ [x x] [~ x ~ ~]"),
)
.bpm(100, '4/4')`
} />
...
//
stack(
  sound("current.hhat") .clip(true),
  sound("sd") .struct("~ x ~ x"),
  sound("bd") .struct(current.bass),
)
.bpm(100, '4/4')`
} />
...
<MiniRepl tune={
`stack(
  sound("oh") .velocity(0.8) .struct("x!8"),
  sound("sd") .velocity(1.0) .struct("~ x ~ x"),
  sound("bd") .velocity(0.8) .struct("x ~ x ~"),
  sound("cp") .velocity(0.8) .struct("~ x ~ x") .hush(),
)
...
<MiniRepl tune={
`stack(
  sound("sd") .struct("~ x ~ <[x ~] [x x]>"),
  sound("sd") .struct( cat("~ x ~ x", "~ x ~ [x x]") ) .gain(0),
)
.bpm(100, '4/4')`
} />
...
`stack(
  sound("sd") .struct("~ x ~ <[x ~]!3 [x x]>"),
  sound("bd") .struct( cat("[x x] [~ x] [x ~ ~ x] [~ x]", 
...
//
stack(
  sound("hh:1") .struct( flatcat(current.half) ),
  sound("oh")   .struct( flatcat(current.open) ),
  sound("sd")   .struct("~ x ~ x"),
  sound("bd")   .struct( flatcat(current.bass) ),
)
.bpm(100, '4/4')`
} />
...
<MiniRepl tune={
`stack(
  sound("sd") .struct("[x x x x] [x x x x] [x x] [x x]") .gain(0.7) .every( 2, $ => $.gain("0.7 0.8 0.9 1") ),
)
.bpm(100, '4/4')`
} />
...
//
var ride = {
  edge: ($) => $ .sound("ride_edge") .velocity(0.9) .clip(true),
  cup:  ($) => $ .sound("ride_cup")  .velocity(1.5) .clip(true),
}
stack(
  sound("__") .struct(flatcat(current.ride)) .apply( $=> ride.cup($) ) .every( 2, $=> ride.edge($) ),
  sound("sd") .struct("~ x ~ x"),
  sound("bd") .struct(flatcat(current.bass)),
)
.bpm(100, '4/4')`
...
Note how each Ride sound has been given its own definition with individual velocity settings. The `sound("__")` is 
just a placeholder without any actual sound. The `apply` function is very similar to the `every` function, but 
doesn't need a condition: It simply applies a function to what's been defined before (and in this case it's applying 
...
function part(instrument, fallback = [silence, silence]) { return flatcat(current[instrument] || fallback) }
//
stack(
  sound("bd") .velocity(0.8) .struct(part('bass')),
  sound("sd") .velocity(1.0) .struct(part('snare',["~ x ~ x","~ x ~ x"])),
  sound(part('hhat')) .velocity(0.8) .clip(true),
  sound("ride_cup") .velocity(0.8) .clip(true) .struct(part('ride')),
  sound("ht") .velocity(0.5) .speed(1.2) .struct(part('high1')),
  sound("ht") .velocity(0.5) .speed(1.0) .struct(part('high2')),
  sound("mt") .velocity(0.5) .speed(1.2) .struct(part('mid')),
  sound("lt") .velocity(0.5) .speed(1.0) .struct(part('low')),
)
.bpm(100, '4/4')`
} />
...
defined. All instruments except for the Snare then simply play two bars of `silence`, which is a pre-defined 
pattern in Strudel. You'll notice that I'm using `silence` for empty bars as well, so I can ensure that all 
patterns have exactly two bars length.
...
them stand out visually (the additional brackets have no actual effect). 

Now that we have so many patterns to chose from, it makes sense to put them in a nice arrangement. 
...
function part(instrument, fallback = [silence, silence]) { return flatcat(song.map($ => $[instrument] || fallback)) }
//
stack(
  sound("bd") .velocity(0.8) .struct(part('bass')),
  sound("sd") .velocity(1.0) .struct(part('snare',["~ x ~ x","~ x ~ x"])),
...
<MiniRepl tune={
`stack(
  sound("bd") .n(4) .struct("x x"),
  sound("piano") .note("c1") .struct("x x")
)
.gain(0.5)`
} />
...
stack(
  sound("bd") .n(3) .struct("x x"),
  sound("piano") .note("c4") .struct("x x")
)
.gain(0.5)`
} />
...
<MiniRepl tune={`
sound("piano") .note("a2 b2 c3 d3 e3 f3 g3@2") .struct("x x x x x x x@2") 
.gain(0.5)`
} />
...
<MiniRepl tune={`
"a2 b2 c3 d3 e3 f3 g3@2" .note() .sound("piano")
.gain(0.5)`
} />
...
<MiniRepl tune={
"a2 b2 c3 d3 e3 f3 g3@2" .note() .sound("bd:3")
.clip(true) .gain(0.3)
} />
...
If you're interested in what's going on in the script, look [here](https://gist.githubusercontent.com/larkob/4c933946b3e2f870b48d149ba11e8be7/raw/bb25988d73d9f6fed74be6a883e1ebe5dbd4a140/strudel_helpers.js)
